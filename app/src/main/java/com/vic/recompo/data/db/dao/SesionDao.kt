...
interface SesionDao {
    ...
    @Query("SELECT * FROM Sesion WHERE estado IN ('PREPARADA', 'EN_CURSO') ORDER BY CASE estado WHEN 'EN_CURSO' THEN 0 ELSE 1 END")
    fun getActivas(): Flow<List<Sesion>>

    @Query("SELECT * FROM Sesion WHERE tipoSesionId = :tipoSesionId AND estado = 'PREPARADA' LIMIT 1")
    suspend fun getPreparadaByTipo(tipoSesionId: Long): Sesion?

    @Query("SELECT * FROM Sesion WHERE tipoSesionId = :tipoSesionId AND estado = 'COMPLETADA' ORDER BY fechaEjecutada DESC LIMIT :limit")
